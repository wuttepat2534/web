...
$q_1 = dd_q('SELECT COUNT(t_id) AS count_bonus, SUM(t_bonus) AS sum_bonus FROM transfergame_tb WHERE t_date_create >= ? AND t_date_create <= ? AND t_bonus > ?', [$start_date, $end_date, 0]);
$row_1 = $q_1->fetch(PDO::FETCH_ASSOC);

$sum_bonus = $row_1['sum_bonus'];
$count_bonus = $row_1['count_bonus'];
?>
<!DOCTYPE html>
<html>
<head>
  ...
  <div class="wrapper">
    ...
    <div class="content-wrapper">
      <section class="content-header pt-4 pb-4">
        <h1 style="font-size: 30px;">รายการโบนัส 
        ...
        <div class="row">
          ...
          <div class="col-sm-6">
            <div class="card card-primary card-outline">
              <div class="card-body box-profile">
                <div class="text-center">
                  <h2><?=number_format($count_bonus, 0)?></h2>
                  <p>รายการโบนัสทั้งหมด</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="row">
          <div class="col-sm-12">
            <button type="button" class="btn btn-success btn-block btn-lg">
              <i class="fas fa-plus"></i> เพิ่มรายการโบนัส
            </button>
          </div>
        </div>
      </section> -->
...
<script type="text/javascript">
  $(function () {
    $('#tb_data').DataTable({
      ...
      "processing": true,
      "serverSide": true,
      "ajax": "./server-side/server_bonus.php?startdate=<?=$start_date?>&enddate=<?=$end_date?>"
    });
  });
  ...
  $("#btn_search").click(function(e) {
    ...
    window.location = './bonus?start_date='+_dateStart+'&end_date='+_dateEnd;
});
</script>
</html>